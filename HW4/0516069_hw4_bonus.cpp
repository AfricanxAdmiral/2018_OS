...
void* mean_filter(void* rank)
{
	long my_rank = (long)rank;
        for (int j = 0+my_rank; j<imgHeight; j+=thread_count) {
	...
	for (int k = 0; k<5; k++){
		...
		for (int j = 0; j<imgHeight; j++) {
			for (int i = 0; i<imgWidth; i++){
		        ...
		        pthread_create(&thread_handles[i], NULL, mean_filter, (void*)(long)i);

		//apply the Mean filter to the image
		for (int j = 0; j<imgHeight; j+=thread_count) {
			for (int i = 0; i<imgWidth; i++){
				pic_mean[j*imgWidth + i] = MeanFilter(i, j);		
			}
		}
		...
		if (counter == thread_count - 1) {
                ...
                for (int i = 1; i < thread_count; i++)
		        pthread_create(&thread_handles[i], NULL, sobel_filter, (void*)(long)i);*/
